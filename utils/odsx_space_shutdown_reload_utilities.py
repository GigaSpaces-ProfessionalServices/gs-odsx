...
def undeployProcessingUnit(managerHost, puId,isSpace,isSecure=False,username=None,password=None):
    ...
    logger.debug("undeployProcessingUnit() response : "+str(response))
...
def undeploySpace(managerHost, statefulPUList,isSecure=False,username=None,password=None):
    logger.info("undeployStatefulPUs() : start")
    ...
    logger.info("undeployStatefulPUs() : end")
...
def undeployFeeders(isSecure=False):
    ...
    for dataArr in feederPuList:
        ...
        if(str(name).casefold().__contains__("db2")):
            isDB2Feeder = True
        if(str(name).casefold().__contains__("mssq")):  
            isMSSQLFeeder = True
...
def validateTierSpace(managerHost,tierSpace,isSecure=False,username=None,password=None):
    ...
    #print("1. totalPrimaryCount -> "+str(totalPrimaryCount))
    #verboseHandle.printConsoleInfo("Verifying primary count.........")
    while(count < 3):
        #print("isPrimaryStable -> "+str(isPrimaryStable))
        ...
        #print("totalPrimaryCount = getAllObjPrimaryCount(managerHost, space_name) => "+str(totalPrimaryCount))
        #print("tempPrimaryCount -> "+str(tempPrimaryCount))
        #print("totalPrimaryCount -> "+str(totalPrimaryCount))
        ...
        #print("isPrimaryStable 22222-> "+str(isPrimaryStable))
        time.sleep(5)
