...
def getPortNotExistInMysqlFeeder(port):
    logger.info("getPortIfNotExist()")
    try:
        feederList = ''
        ...
        cnx = sqlite3.connect(db_file)
        myCursor = cnx.cursor()
        logger.info("SELECT * FROM mysql_host_port where port="+str(port))
        myCursor.execute("SELECT * FROM mysql_host_port where port="+str(port))
        myresult = myCursor.fetchall()
        logger.info("myresult :"+str(myresult))
        if(len(str(myresult))>2):
            for x in myresult:
