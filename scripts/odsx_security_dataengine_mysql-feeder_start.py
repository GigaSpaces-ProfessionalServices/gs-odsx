...
from utils.odsx_db2feeder_utilities import getMySqlQueryStatusFromSqlLite, getUsernameByHost, getPasswordByHost
...
def displayMysqlFeederShFiles():
    ...
    logger.info("sourceDB2FeederShFilePath: "+str(sourceMysqlFeederShFilePath))
    counter=1
    directory = os.getcwd()
...
def inputParam():
    ...
    if(str(inputChoice)=='1'):
        inputNumberToStart = str(userInputWrapper(Fore.YELLOW+"Enter serial number to start mysql-feeder : "+Fore.RESET))
        if(len(str(inputNumberToStart))==0):
            inputNumberToStart = str(userInputWrapper(Fore.YELLOW+"Enter serial number to start mysql-feeder : "+Fore.RESET))
        proceedToStartMysqlFeeder(inputNumberToStart)
    if(len(str(inputChoice))==0):
        confirmAll = str(userInputWrapper(Fore.YELLOW+"Do you want to start all mysql-feeders y/N [N]: "+Fore.RESET))
        if(confirmAll.casefold()!='y' and confirmAll.casefold()!='yes'):
            return
        elements = len(fileNameDict)
        for i in range (1,elements+1):
            proceedToStartMysqlFeeder(str(i))
...
def proceedToStartMysqlFeeder(fileNumberToStart):
    logger.info("proceedToStartMysqlFeeder()")
...
if __name__ == '__main__':
    logger.info("odsx_dataengine_mysql-feeder_start")
    verboseHandle.printConsoleWarning("Menu -> DataEngine -> Mysql-Feeder -> Start")
    username = ""
    password = ""
    ...
    try:
        ...
        if(len(str(managerHost))>0):
            ...
            if(len(str(gs_space_dictionary_obj))>2):
                if len(sys.argv) > 1 and sys.argv[1] != "m":
                    proceedToStartMysqlFeederWithName(sys.argv[1])
                else:
                    inputParam()
            else:
        ...
        verboseHandle.printConsoleError("Eror in odsx_mysql-feeder_start : "+str(e))
        handleException(e)
