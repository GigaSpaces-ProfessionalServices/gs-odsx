...
def validateUserInput():
    ...
    objectMgmtTableInput = str(
        userInputWithEscWrapper(Fore.YELLOW + "Select object to get memory count type \n or exit [99] :" + Fore.RESET))
    ...
    if objectMgmtTableInput.isnumeric() == True and objectMgmtTableInput!="99":
        ...
        if objectMgmtTableInput > 0 and objectMgmtTableInput in dataSpaceDict:
            ...
            whereCondition = str(
                userInputWithEscWrapper(Fore.YELLOW + "condition to search (eg column<'5' and column>'1') :" + Fore.RESET))
            #displaySummary()
...
def memoryCount():
    ...
    # print(getData())
    response = requests.get('http://' + objectMgmtHost + ':7001/inmemory/count', params=getData(),
                             headers={'Accept': 'application/json'})
