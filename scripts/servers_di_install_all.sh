...
function installFlink() {
  ...
  mkdir -p $gigalogpath/di-iidr/di-flink
  ...
  sed -i 's|latest-di-flink|latest-flink|g' $HOME/.config/systemd/user/di-flink-jobmanager.service
  sed -i 's|latest-di-flink|latest-flink|g' $HOME/.config/systemd/user/di-flink-taskmanager.service
  sed -i "s|FLINK_LOG_DIR=[^ ]*|FLINK_LOG_DIR=$gigalogpath/di-iidr/di-flink|g" $HOME/.config/systemd/user/di-flink-jobmanager.service
  ...
  systemctl --user restart di-flink-jobmanager.service
...
function installDIMatadata {
  ...
  mkdir -p $gigalogpath/di-mdm       # Fixed: was $gigapathlogs (typo)
  ...
  tee $gigapath/di-mdm/$extracted_folder_mdm/utils/global-${currentHost}.env > /dev/null << ENVEOF
...
function installDITransformations {
  ...
  mkdir -p $gigapath/di-transformations
  mkdir -p $gigalogpath/di-transformations
  cp $installation_path/$installation_file $gigapath/di-transformations
  info "\nExtracting zip file...\n"
  tar -xzf $gigapath/di-transformations/$installation_file --directory $gigapath/di-transformations/
  ...
  tee $gigapath/di-transformations.properties > /dev/null << TRANEOF
...
TRANEOF
  ...
  # Patch log path in service file BEFORE install_new_version.sh deploys it to $HOME/.config/systemd/user/
  sed -i "s|latest-di-transformations/logs/di-transformations.log|${gigalogpath}/di-transformations/di-transformations.log|g" \
      $gigapath/di-transformations/$extracted_folder_transformations/config/di-transformations.service
  bash $gigapath/di-transformations/$extracted_folder_transformations/utils/install_new_version.sh $gigapath/di-transformations.properties
  restorecon $HOME/.config/systemd/user/di-transformations.service 2>/dev/null || true
  systemctl --user daemon-reload
...
if [ -f $sourceInstallerDirectory/zk/$zookeeper_service_file ]; then
  cp $sourceInstallerDirectory/zk/$zookeeper_service_file $HOME/.config/systemd/user/
elif [ -f $home_dir_sh/install/zookeeper/$zookeeper_service_file ]; then
  cp $home_dir_sh/install/zookeeper/$zookeeper_service_file $HOME/.config/systemd/user/
else
...
if [[ $id != 4 ]]; then
  ...
  if [ -f $sourceInstallerDirectory/kafka/$kafka_service_file ]; then
    cp $sourceInstallerDirectory/kafka/$kafka_service_file $HOME/.config/systemd/user/
    ...
    cp $home_dir_sh/install/kafka/$kafka_service_file $HOME/.config/systemd/user/
  else
  ...
  if [ $id == 1 ]; then
    ...
    systemctl --user enable --now odsxkafka.service
    systemctl --user daemon-reload
    installDIMatadata
