...
from scripts.odsx_tieredstorage_updatecachepolicy import listSpacesOnServer, getManagerHost, listGSC, handleException
...
def proceedForNewBackUpRollingUpdate(gscNumberToBeRollingUpdate):
    ...
    try:
        ...
        newbackUpRestartresponse = requests.post("http://"+str(managerHostConfig)+":8090/v2/containers/"+str(containerIdToBeRestarted)+"/restart",headers=requestHeader,auth = HTTPBasicAuth(username,password))
        with Spinner():
            time.sleep(5)
...
def proceedForDemotePrimaryContainer(gscNumberToBeRollingUpdate):
    ...
    try:
        ...
        primaryDemoteResponse = requests.post("http://"+str(managerHostConfig)+":8090/v2/spaces/"+str(spaceName)+"/instances/"+str(spaceIdToBeDemoted)+"/demote?maxSuspendTime="+str(maxSuspendTime),headers=requestHeader,auth = HTTPBasicAuth(username,password))
...
def proceedForBackupRollingUpdate(gscNumberToBeRollingUpdate):
    ...
    try:
        ...
        backUpIndividualGSCRestartresponse = requests.post("http://"+str(managerHostConfig)+":8090/v2/containers/"+str(backupContainerId)+"/restart",headers=requestHeader,auth = HTTPBasicAuth(username,password))
...
if __name__ == '__main__':
    ...
    try:
        ...
        #verboseHandle.printConsoleError("Eror in odsx_security_tieredstorage_rollback : "+str(e))
