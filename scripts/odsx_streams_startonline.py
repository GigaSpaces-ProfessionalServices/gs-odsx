...
def exitAndDisplay(isMenuDriven):
    if(isMenuDriven=='m'):
        os.system('python3 scripts/odsx_streams_startonline.py'+' '+isMenuDriven)
    else:
        cliArgumentsStr=''
        for arg in cliArguments:
            cliArgumentsStr+=arg
            cliArgumentsStr+=' '
        os.system('python3 scripts/odsx_streams_startonline.py'+' '+cliArgumentsStr)


if __name__ == '__main__':
    logger.info("odsx_servers_streams_startonline")
    verboseHandle.printConsoleWarning('Servers -> Streams -> Start online')
    args = []
    ...
    try:
        ...
        if(optionMainMenu != 99):
            ...
            if len(streamDict) >= optionMainMenu:
                ...
                verboseHandle.printConsoleWarning("Are you sure want to start selected stream ? [Yes][No][Cancel]")
                choice = str(input(""))
