...
def addNewColumn(diManagerHost):
    ...
    #     logger.error(f"Pipeline '{selected_pipeline}' is INACTIVE. Operation aborted.")
    #     return
    ...
    verboseHandle.printConsoleInfo(f"Selected Space Type Name: {space_type_name}")
    logger.info(f"Selected Space Type Name: {space_type_name}")
    ...
    if not existing_exclude:
        verboseHandle.printConsoleInfo("No fields excluded.")
        return
    ...
    verboseHandle.printConsoleInfo("Fields to add : " + str(fields_to_remove))
    logger.info("Fields to add : " + str(fields_to_remove))

    confirm = userInputWrapper(Fore.YELLOW + "This will stop current running pipeline, Add column to the Space type and reimport new pipeline after adding column. Continue? (yes/no): " + Fore.RESET).strip().lower()
    ...
    for stn in all_space_types:
        ...
        logger.info(f"Unregistering space type: {stn}")
        try:
            ...
            if unreg_response.text.strip() == "success":
                verboseHandle.printConsoleInfo(f"Space type '{stn}' unregistered successfully.")
                logger.info(f"Space type '{stn}' unregistered successfully.")
            else:
                verboseHandle.printConsoleError(f"Failed to unregister space type '{stn}': {unreg_response.text}")
                logger.error(f"Unregister space type response: {unreg_response.text}")
                failed_types.append(stn)
    ...
    try:
        ...
        for space in object_list:
            for obj in space.get("objects", []):
                registered_types.add(str(obj.get("tablename", "")).strip())
        still_present = [stn for stn in all_space_types if stn in registered_types]
        if still_present:
        ...
        verboseHandle.printConsoleError(f"Verification error: {e}")
