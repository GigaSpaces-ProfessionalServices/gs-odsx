...
useradd dbsh
if [ -d "/home/dbsh/cr8/latest_cr8" ]
then
#if [[ "$IS_CR8_INSTALL" = 1 ]]; then
  cd /home/dbsh/
  oldPackageName=$(date +%Y%m%d_%H%M%S)
  mv cr8 cr8_previous_$oldPackageName
  echo "moved the existing packages cr8_previous_$oldPackageName"
fi
mkdir -p /root/misc/
cd /
...
cd /root/misc
cp $installation_path/$installation_file .
