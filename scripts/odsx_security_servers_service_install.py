...
from utils.ods_cluster_config import config_add_service_node, config_get_cluster_airgap, config_get_service_hosts, \
    isInstalledAndGetVersion, config_get_manager_node, config_get_service_list_with_status, getServiceHostFromEnv
...
def execute_ssh_server_manager_install(hostsConfig,user):
    ...
    try:
        ...
        targetDirectory=''
        gsOptionExtFromConfig = str(readValueByConfigObj("app.service.security.gsOptionExt")).replace('[','').replace(']','').replace("'","").replace(', ',',')
        ...
        sourceJar = str(getYamlFilePathInsideFolder(".security.jars.all")).replace('"','').replace('[','').replace(']','')
        msSqlFeederFilePath="."
        ...
        verboseHandle.printConsoleWarning("-------------------HI1-----------------------------------------")
        verboseHandle.printConsoleWarning(str(serviceHostConfig))
        ...
        verboseHandle.printConsoleWarning("---------------------HI---------------------------------------")
            ...
            with ThreadPoolExecutor(hostListLength) as executor:
                for host in host_nic_dict_obj:
                    if (host in serviceHostConfig):
                        executor.submit(installServiceServer,host,host_nic_dict_obj,additionalParam,cefLoggingJarInput,cefLoggingJarInputTarget,db2jccJarInput,db2FeederJarTargetInput,db2jccJarLicenseInput,msSqlFeederFileTarget,sourceJar,springTargetJarInput,None,None,applicativeUser,startSpaceGsc,newZkJarTarget,selinuxEnabled,infraJarInput,infraJarTargetInput)
...
def installServiceServer(host,host_nic_dict_obj,additionalParam,cefLoggingJarInput,cefLoggingJarInputTarget,db2jccJarInput,db2FeederJarTargetInput,db2jccJarLicenseInput,msSqlFeederFileTarget,sourceJar,springTargetJarInput,ldapSecurityConfigInput,ldapSecurityConfigTargetInput,applicativeUser,startSpaceGsc,newZkJarTarget,selinuxEnabled,infraJarInput,infraJarTargetInput):
    try:
        installStatus='No'
        ...
        if installStatus == 'No':
            ...
            commandToExecute="scripts/security_service_install.sh"
            ...
            with Spinner():
                managerWork = str(readValueByConfigObj("app.service.workDir"))
                ...
                if gs_version_17=='true':
                    ...
                    source17Jar = str(getYamlFilePathInsideFolder(".security.17_gs_jars.all")).replace('"','').replace('[','').replace(']','')
...
if __name__ == '__main__':
    logger.info("odsx_security_service-install")
