...
def exitAndDisplay(isMenuDriven):
    if(isMenuDriven=='m'):
        os.system('python3 scripts/odsx_streams_stoponline.py'+' '+isMenuDriven)
    else:
        cliArgumentsStr=''
        for arg in cliArguments:
            cliArgumentsStr+=arg
            cliArgumentsStr+=' '
        os.system('python3 scripts/odsx_streams_stoponline.py'+' '+cliArgumentsStr)

if __name__ == '__main__':
    logger.info("odsx_servers_streams_stoponline")
    verboseHandle.printConsoleWarning('Servers -> Streams -> Stop online')
    args = []
    ...
    try:
        ...
        if(optionMainMenu != 99):
            ...
            if len(streamDict) >= optionMainMenu:
                ...
                verboseHandle.printConsoleWarning("Are you sure want to stop selected stream ? [Yes][No][Cancel]")
                choice = str(input(""))
                        ...
                        if(streamResumeStream.serverip == arguments.host):
                            ...
                            if(arguments.dryrun==True):
                                ...
                                if(str(output).__contains__('True')):
                                    verboseHandle.printConsoleInfo("CR8_ctl.sh found to execute stop stream.")
                                elif(str(output).__contains__('False')):
                                    verboseHandle.printConsoleInfo('CR8_ctl.sh not found to execute stop stream')
                                quit()
