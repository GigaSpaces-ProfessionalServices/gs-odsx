...
from utils.odsx_db2feeder_utilities import getGilboaQueryStatusFromSqlLite, getPasswordByHost, getUsernameByHost
...
def displayMSSQLFeederShFiles():
    ...
    headers = [Fore.YELLOW+"Sr No."+Fore.RESET,
               Fore.YELLOW+"Name of gilboa-feeder file"+Fore.RESET
               ]
    dataTable=[]
...
def listDeployed(managerHost):
    ...
    try:
        ...
        for i in jsonArray:
            if(str(i['name']).__contains__("gilboafeeder")):
                flag = True
...
def inputParam():
    ...
    if(str(inputChoice)=='1'):
        inputNumberToStart = str(userInputWrapper(Fore.YELLOW+"Enter serial number to start gilboa-feeder : "+Fore.RESET))
        if(len(str(inputNumberToStart))==0):
            inputNumberToStart = str(userInputWrapper(Fore.YELLOW+"Enter serial number to start gilboa-feeder : "+Fore.RESET))
        proceedToStartMSSQLFeeder(inputNumberToStart)
    if(len(str(inputChoice))==0):
        confirmAll = str(userInputWrapper(Fore.YELLOW+"Do you want to start all gilboa-feeders y/N [N]: "+Fore.RESET))
