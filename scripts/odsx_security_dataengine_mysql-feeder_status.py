...
def displayMysqlFeederShFiles():
    logger.info("displayMysqlFeederShFiles()")
    global fileNameDict
    ...
    sourceMysqlFeederShFilePath = str(str(sourceInstallerDirectory+".mysql.scripts.").replace('.','/'))
    logger.info("sourceMysqlFeederShFilePath :"+str(sourceMysqlFeederShFilePath))
    counter=1
    directory = os.getcwd()
    ...
    headers = [Fore.YELLOW+"Sr No."+Fore.RESET,
               Fore.YELLOW+"Name of mysql-feeder file"+Fore.RESET
               ]
    dataTable=[]
...
def listDeployed(managerHost):
    ...
    try:
        ...
        for i in jsonArray:
            if(str(i['name']).__contains__("mysqlfeeder")):
                flag = True
        if(len(jsonArray) == 0 or flag == False):
            ...
            for data in jsonArray:
                ...
                queryStatus = str(getMySqlQueryStatusFromSqlLite(str(data["name"]))).replace('"', '')
                ...
                if (str(data["name"]).__contains__("mysqlfeeder")):
                    ...
                    os.chdir(sourceMysqlFeederShFilePath)
                    for file in glob.glob("load_*.sh"):
                        puName = str(str(data["name"])).replace('mysqlfeeder_', 'load_').casefold()
                        puName = puName + ".sh"
                    ...
                    logger.info("UPDATE mysql_host_port SET host='"+str(hostId)+"' where feeder_name like '%"+str(data["name"])+"%' ")
                    mycursor = cnx.execute("UPDATE mysql_host_port SET host='"+str(hostId)+"' where feeder_name like '%"+str(data["name"])+"%' ")
...
def inputParam():
    ...
    if(str(inputChoice)=='1'):
        inputNumberToStatus = str(userInputWrapper(Fore.YELLOW+"Enter serial number to status mysql-feeder : "+Fore.RESET))
        if(len(str(inputNumberToStatus))==0):
            inputNumberToStatus = str(userInputWrapper(Fore.YELLOW+"Enter serial number to get status mysql-feeder : "+Fore.RESET))
        proceedToGetStatusMysqlFeeder(gs_space_dictionary_obj.get(str(inputNumberToStatus)))
    #if(len(str(inputChoice))==0):
    #    elements = len(fileNameDict)
    #    for i in range (1,elements+1):
    #        proceedToGetStatusMysqlFeeder(gs_space_dictionary_obj.get(str(i)))
...
def sqlLiteGetHostAndPortByFileName(puName):
    ...
    try:
        ...
        logger.info("SQL: SELECT host,port FROM mysql_host_port where feeder_name like '%"+str(puName)+"%' ")
        mycursor = cnx.execute("SELECT host,port FROM mysql_host_port where feeder_name like '%"+str(puName)+"%' ")
...
def proceedToGetStatusMysqlFeeder(puName):
    logger.info("proceedToGetStatusMysqlFeeder() " + puName)
    # puName = gs_space_dictionary_obj.get(str(fileNumberToStatus))
    queryStatus = str(getMySqlQueryStatusFromSqlLite(puName)).replace('"', '')
    verboseHandle.printConsoleInfo(puName + " : " + queryStatus)

def getMySqlQueryStatusFromSqlLite(feederName):
    logger.info("getQueryStatusFromSqlLite() shFile : "+str(feederName))
    try:
        db_file = str(readValueByConfigObj("app.dataengine.mysql-feeder.sqlite.dbfile")).replace('"','').replace(' ','')
        cnx = sqlite3.connect(db_file)
        logger.info("Db connection obtained."+str(cnx))
        logger.info("CREATE TABLE IF NOT EXISTS mysql_host_port (file VARCHAR(50), feeder_name VARCHAR(50), host VARCHAR(50), port varchar(10))")
        ...
        cnx.commit()
        logger.info("SQL : SELECT host,port FROM mysql_host_port where feeder_name like '%"+str(feederName)+"%' ")
        mycursor = cnx.execute("SELECT host,port FROM mysql_host_port where feeder_name like '%"+str(feederName)+"%' ")
        myresult = mycursor.fetchall()
        cnx.close()
        host = ''
        port = ''
...
if __name__ == '__main__':
    logger.info("odsx_security_dataengine_mysql-feeder_status")
    verboseHandle.printConsoleWarning("Menu -> DataEngine -> Mysql-Feeder -> Status")
    username = ""
    password = ""
    ...
    try:
        ...
        verboseHandle.printConsoleError("Eror in odsx_data-engine_mysql-feeder_status : "+str(e))
        handleException(e)
