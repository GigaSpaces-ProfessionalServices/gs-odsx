...
def createGSCInputParam(managerNodes,spaceNodes,managerHostConfig):
    ...
    try:
        ...
        confirmCreateGSC = str(input("Do you want to create GSC ? (y/n) ["+confirmCreateGSC+"] :"+Fore.RESET))
        ...
        if(confirmCreateGSC=='y'):
            ...
            memoryGSCValue = str(readValuefromAppConfig("app.spacejar.creategsc.gscmemory"))#str(input(Fore.YELLOW+"Enter memory to create gsc [12g] :"+Fore.RESET))
            memoryGSC = str(input("Enter memory to create gsc ["+memoryGSCValue+"] :"+Fore.RESET))
            if(len(memoryGSC)==0):
                memoryGSC=memoryGSCValue

            zoneGSCValue = str(readValuefromAppConfig("app.spacejar.creategsc.gsczone"))#str(input(Fore.YELLOW+"Enter zone :"+Fore.RESET))
...
def displaySummaryOfInputParam(confirmCreateGSC):
    try:
        ...
        if(backUpRequired==1):
            ...
            haStatus = False
        verboseHandle.printConsoleInfo("SLA [HA] ? (y/n) :"+str(str(haStatus)))
...
def proceedForTieredStorageDeployment(managerHostConfig,confirmCreateGSC):
    ...
    try:
        ...
        pathOfSourcePU = str(input(Fore.YELLOW+"Name of resource will be deploy ["+str(pathOfSourcePUValue)+"] : "+Fore.RESET))
        while(len(str(pathOfSourcePU))==0):
