...
sleep 5
rm -rf $GS_HOME
rm -rf setenv.sh gs install install.tar $gigaDir/giga* $gigaData/* $gigaWork/* /usr/local/bin/start_gs*.sh /usr/local/bin/stop_gs*.sh /etc/systemd/system/gs*.service
