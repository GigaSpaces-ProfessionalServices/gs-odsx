...
from utils.odsx_db2feeder_utilities import getPortNotExistInMysqlFeeder, getPasswordByHost, getUsernameByHost
...
def deleteMysqlEntryFromSqlLite(puName):
    logger.info("deleteMysqlEntryFromSqlLite()")
    try:
        ...
        logger.info("SQL : DELETE FROM mysql_host_port where feeder_name like '%"+str(puName)+"%'")
        cnx.execute("DELETE FROM mysql_host_port where feeder_name like '%"+str(puName)+"%'")
        cnx.commit()
        cnx.close()
    except Exception as e:
        handleException(e)
...
def proceedForPersistUndeploy(spaceTobeUndeploy):
    ...
    if(response.status_code==200):
       ...
       # deleteMysqlEntryFromSqlLite(spaceTobeUndeploy)
    else:
...
def feeder_undeploy(puTobeUndeploy):
    ...
    if(response.status_code==202):
        ...
        deleteMysqlEntryFromSqlLite(puTobeUndeploy)
        verboseHandle.printConsoleInfo(" Undeploy  : "+str(puTobeUndeploy)+"   Status : "+str(status))
        proceedForPersistUndeploy(puTobeUndeploy)

def newInstallMysqlFeeder(tableName):
    global newInstall
    newInstall=[]
    directory = os.getcwd()
    ...
    global activefeeder
    activefeeder=[]
    exitsFeeder = fileName.replace('load','mysqlfeeder').replace('.sh','').casefold()
...
def updateAndCopyJarFileFromSourceToShFolder(puName):
    ...
    filePrefix = fileName
    targetJarFile=sourceMysqlFeederShFilePath+fileName+"_"+puName+'.jar'
    userCMD = os.getlogin()
    if userCMD == 'ec2-user':
        cmd = "sudo cp "+sourceMysqlJarFilePath+' '+sourceMysqlFeederShFilePath+fileName+"_"+puName+'.jar'
    else:
        cmd = "cp "+sourceMysqlJarFilePath+' '+sourceMysqlFeederShFilePath+fileName+"_"+puName+'.jar'
...
def createMysqlEntryInSqlLite(puName, file, restPort):
    ...
    try:
        ...
        db_file = str(readValueByConfigObj("app.dataengine.mysql-feeder.sqlite.dbfile")).replace('"','').replace(' ','')
        cnx = sqlite3.connect(db_file)
        cnx.execute("INSERT INTO mysql_host_port (file, feeder_name, host, port) VALUES ('load_"+str(file).upper()+".sh', '"+str(puName)+"','"+str(hostId)+"','"+str(restPort)+"')")
        cnx.commit()
        cnx.close()
    except Exception as e:
        handleException(e)
...
def proceedToDeployPUInputParam(tableName):
    ...
    newInstallMysqlFeeder(tableName)
    global partition
    partition='1'
...
def sqlLiteGetHostAndPortByFileName(puName):
    ...
    try:
        ...
        logger.info("SQL : SELECT host,port,file FROM mysql_host_port where feeder_name = '"+str(puName)+"' ")
        mycursor = cnx.execute("SELECT host,port,file FROM mysql_host_port where feeder_name = '"+str(puName)+"' ")
...
def proceedToStartMysqlFeederWithName(puName):
    logger.info("proceedToStartMysqlFeederWithName()")
    ...
    cmd = str(sourceMysqlFeederShFilePath)+'/'+shFileName+' '+host+" "+port
    logger.info("cmd : "+str(cmd))
    os.system(cmd)
...
if __name__ == '__main__':
    verboseHandle.printConsoleWarning('Menu -> DataEngine -> Mysql-Feeder -> Schema change')
    global username
    global password
    #global tableName
    global sourceMysqlFeederShFilePath
    ...
    puName = "mysqlfeeder_" + tableName
    feeder_undeploy(puName)
    proceedToDeployPUInputParam(tableName)
    #Start the feeder
    proceedToStartMysqlFeederWithName(puName)
