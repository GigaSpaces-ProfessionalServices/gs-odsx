...
def executeService(host):
    ...
    with Spinner():
        ...
        if (output == 0):
            verboseHandle.printConsoleInfo("Service odsxadabas started successfully on " + str(host))
        else:
            verboseHandle.printConsoleError("Service odsxadabas failed to start on " + str(host))


def startAdabusService(args):
    try:
        ...
        if(inputChoice=='1'):
            ...
            if len(nodes) == 0:
                ...
                choice = str(userInputWrapper(Fore.YELLOW + "Are you sure, you want to start adabus service for [" + str(
...
if __name__ == '__main__':
    verboseHandle.printConsoleWarning("Menu -> MQ Connector -> Adabas Service -> Start")
    args = []
    args = myCheckArg()
