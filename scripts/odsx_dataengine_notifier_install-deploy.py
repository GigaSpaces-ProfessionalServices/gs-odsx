...
def proceedToCreateGSC(zoneGSC,newGSCCount,feederType):
    ...
    commandToExecute = "cd; home_dir=$(pwd); source $home_dir/setenv.sh;$GS_HOME/bin/gs.sh container create --count="+str(numberOfGSC)+" --zone="+str(zoneGSC)+" --memory="+str(memoryGSC)+" "+str(SpaceHighestAvailableHost)+" | grep -v JAVA_HOME"
