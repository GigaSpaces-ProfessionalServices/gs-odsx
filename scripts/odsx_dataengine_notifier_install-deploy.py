...
def proceedToCreateGSC(zoneGSC,newGSCCount):
    ...
    host = spaceNodes[idx]
    commandToExecute = "cd; home_dir=$(pwd); source $home_dir/setenv.sh;$GS_HOME/bin/gs.sh container create --count="+str(numberOfGSC)+" --zone="+str(zoneGSC)+" --memory="+str(memoryGSC)+" "+str(os.getenv(host.ip))+" | grep -v JAVA_HOME"
