...
function gsCreateGSServeice {
  ...
  cmd="sleep 60;$GS_HOME/bin/gs.sh --username=$username --password=$password container create --count=$gscCount --zone=$zoneGSC --memory=$memoryGSC "`hostname`""
...
echo "param15"$username
echo "param16"$password
