...
def myCheckArg(args=None):
    parser = argparse.ArgumentParser(description='Script to start DataValidator Scheduler Service')
...
def startDataValidationSchedulerService(args):

    logger.info("startDataValidationSchedulerService()")

    confirmMsg = Fore.YELLOW + "Are you sure, you want to start DataValidator Scheduler service ? (Yes/No) [Yes]:" + Fore.RESET
...
if __name__ == '__main__':
    verboseHandle.printConsoleWarning("Menu -> Datavalidator -> Scheduler Service -> Enable")
    args = []
    args = myCheckArg()
    signal.signal(signal.SIGINT, signal_handler)
    startDataValidationSchedulerService(args)
