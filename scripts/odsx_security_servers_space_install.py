...
from utils.ods_list import configureMetricsXML, getPlainOutput, validateRPMS, getManagerHostFromEnv, configureMetricsProperties, configureOtelIdentity, configureCefLogging
...
#        springLdapJarInput = str(getYamlFilePathInsideFolder(".security.jars.springldapjar")).replace('[','').replace(']','')
       ...
       # ldapSecurityConfigTargetInput = str(readValuefromAppConfig("app.manager.security.config.ldap.target.file"))
        ...
        verboseHandle.printConsoleWarning(str(spaceHostConfig))
    ...
    try:
        ...
        if installStatus == 'No':
            ...
            with Spinner():
                ...
                #print(outputShFile)
                # The CEF jar is deployed by configureCefLogging() below, which
                # also resolves the handler's log path - a jar on its own still
                # fails with "Failed to create directories: /gigalogs/CEF" off
                # the default layout.
               ...
               # executeRemoteCommandAndGetOutputValuePython36(host, user,"cp "+getYamlFilePathInsideConfigFolder("..security.sqljdbc")+" "+msSqlFeederFileTarget)
                ...
                scp_upload(host, get_ssh_user(), spaceWorkSrc, spaceWorkTarget)
