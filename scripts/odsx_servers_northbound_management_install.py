...
def proceedForEnvHostConfiguration(sourceNbConfFile):
    logger.info("proceedForEnvHostConfiguration()")
    userCMD = os.getlogin()
    if userCMD == 'ec2-user':
        cmd = 'sudo cp /dbagigashare/current/NB/MANAGEMENT/nb.conf.template '+sourceNbConfFile+';sudo chown ec2-user:ec2-user '+sourceNbConfFile # Creating .tar file on Pivot machine
    else :
        cmd = 'cp /dbagigashare/current/NB/MANAGEMENT/nb.conf.template '+sourceNbConfFile # Creating .tar file on Pivot machine
    with Spinner():
    ...
    lines = update_app_config_file_shared("consul_servers=".upper(), getNBApplicativeHostFromEnv(), None,sourceNbConfFile)
    lines = update_app_config_file_shared("influxdb_servers=".upper(), validateAndConfigureInfluxdb(), lines,sourceNbConfFile)
    lines = update_app_config_file_shared("grafana_servers=".upper(), validateAndConfigureGrafana(), lines,sourceNbConfFile)
    lines = update_app_config_file_shared("gridui_servers=".upper(), getNBOPSManagerHostFromEnv(), lines,sourceNbConfFile)
    lines = update_app_config_file_shared("opsmanager_servers=".upper(), getNBOPSManagerHostFromEnv(), lines,sourceNbConfFile)
    update_app_config_file_shared("CONSUL_REPLICA_NUMBER=".upper(), str(len(getNBApplicativeHostFromEnv().split(","))), lines,sourceNbConfFile)
...
def setConfProperties():
    ...
    if consul_replica_number == "":
        consul_replica_number = str(nbConfig.get("CONSUL_REPLICA_NUMBER"))
    logger.info("consul_replica_number : "+str(consul_replica_number))
    ...
    if applicationServerFlag == "n":
        ...
        if(len(str(grafana_servers))>0 and len(str(influxdb_servers))>0):
            ...
            gridui_servers= str(nbConfig.get("GRIDUI_SERVERS")).replace('"','')
...
def cleanNbConfig():
    userCMD = os.getlogin()
    if userCMD == 'ec2-user':
        cmd = 'sudo rm -f /dbagigashare/current/NB/MANAGEMENT/nb.conf'
    else:
        cmd = 'rm -f /dbagigashare/current/NB/MANAGEMENT/nb.conf'
    with Spinner():
        status = os.system(cmd)
