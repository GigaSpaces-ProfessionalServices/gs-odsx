...
def update_app_config_file_shared(linePatternToReplace, value, lines1,sourceNbConfFile):
    if lines1 == None:
...
def proceedForEnvHostConfiguration(sourceNbConfFile):
    ...
    userCMD = os.getlogin()
    if userCMD == 'ec2-user':
        cmd = 'sudo cp /dbagigashare/current/NB/APPLICATIVE/nb.conf.template '+sourceNbConfFile+';sudo chown ec2-user:ec2-user '+sourceNbConfFile # Creating .tar file on Pivot machine
    else :
        cmd = 'cp /dbagigashare/current/NB/APPLICATIVE/nb.conf.template '+sourceNbConfFile # Creating .tar file on Pivot machine
    with Spinner():
    ...
    if(confirmInstallation == 'y'):
        ...
        if(confirmServerInstall=='y'):
            ...
            for hostip in nbConfig.get("CONSUL_SERVERS").replace(" ", "").replace("\"", "").split(","):
                ...
                logger.info("Building .tar file : tar -cvf install/install.tar /dbagigashare/current/")
                cmd = 'tar -cvf install/install.tar /dbagigashare/current/' # Creating .tar file on Pivot machine
                with Spinner():
                    status = os.system(cmd)
                    logger.info("Creating tar file status : "+str(status))
