...
def proceedToDeployPUInputParam(managerHost):
    ...
    global resourceName
    resourceName = str(
        input(Fore.YELLOW + "Enter name of PU to deploy [consumer_hb_jp_g_garage1-dih-consumer.war] :" + Fore.RESET))
    if (len(str(resourceName)) == 0):
        resourceName = 'consumer_hb_jp_g_garage1-dih-consumer.war'
    logger.info("nameOfPU :" + str(resourceName))

    global resourcePath
    resourcePath = str(input(Fore.YELLOW + "Enter path of PU to deploy [/dbagigawork] :" + Fore.RESET))
    if (len(str(resourcePath)) == 0):
        resourcePath = '/dbagigawork'
