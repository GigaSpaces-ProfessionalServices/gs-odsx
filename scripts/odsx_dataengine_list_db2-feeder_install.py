...
import os,time,subprocess,requests, json, math, glob
...
def updateAndCopyJarFileFromSourceToShFolder(puName):
    ...
    verboseHandle.printConsoleInfo("done copying..")
...
def proceedToDeployPU():
    ...
    for file in glob.glob("*.sh"):
        ...
        data = getDataPUREST(resource,puName,zoneGSC)
        logger.info("data : of pu :"+puName)
...
if __name__ == '__main__':
    verboseHandle.printConsoleWarning('Menu -> DataEngine -> List -> DB2-Feeder -> Install')
    try:
