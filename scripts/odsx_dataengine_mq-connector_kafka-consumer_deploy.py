...
def proceedToCreateGSC():
    ...
    for host in spaceNodes:
        ...
        commandToExecute = "cd; home_dir=$(pwd); source $home_dir/setenv.sh;$GS_HOME/bin/gs.sh container create --count="+str(numberOfGSC)+" --zone="+str(zoneGSC)+" --memory="+str(memoryGSC)+" --vm-option -Dspring.profiles.active=connector --vm-option -Dpipeline.config.location="+str(dPipelineLocationTarget)+" "+str(os.getenv(host.ip))+" | grep -v JAVA_HOME"
        verboseHandle.printConsoleInfo("Creating container : count="+str(numberOfGSC)+" zone="+str(zoneGSC)+" memory="+str(memoryGSC)+" --vm-option -Dspring.profiles.active=connector --vm-option -Dpipeline.config.location="+str(dPipelineLocationTarget)+" host="+str(os.getenv(host.ip)))
