...
def removeInputUserAndHost():
    ...
    try:
        global user
        global host
        user = str(userInputWrapper(Fore.YELLOW+"Enter user to connect to Grafana [root]:"+Fore.RESET))
        if(len(str(user))==0):
            user="root"
...
def executeCommandForUnInstall():
    ...
    try:
        ...
        if(len(nodes)>0):
            confirmUninstall = str(userInputWrapper(Fore.YELLOW+"Are you sure want to remove Grafana servers ["+nodes+"] (y/n) [y]: "+Fore.RESET))
