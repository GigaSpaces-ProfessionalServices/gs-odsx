...
mv $installation_path/APPLICATIVE/nb.conf $targetDir/nb-infra/

echo "copying ssl files to $targetDir/nb-infra/ssl"
pfxFiles=$(ls $installation_path/APPLICATIVE/ssl/*.pfx 2> /dev/null | wc -l)
echo "pemFiles:"$pfxFiles
if [[ $pfxFiles -gt 0 ]]
then
    echo "Copying .pfx file"
    cp $installation_path/APPLICATIVE/SSL/*.pfx $targetDir/nb-infra/ssl/
fi
crtFiles=$(ls $installation_path/APPLICATIVE/SSL/*.crt 2> /dev/null | wc -l)
echo "crtFiles:"$crtFiles
if [[ $crtFiles -gt 0 ]]
then
    echo "Copying .crt file"
    cp $installation_path/APPLICATIVE/SSL/*.crt $targetDir/nb-infra/ssl/
fi
pemFiles=$(ls $installation_path/APPLICATIVE/SSL/*.pem 2> /dev/null | wc -l)
echo "pemFiles:"$pemFiles
if [[  $pemFiles -gt 0 ]]
then
    echo "Copying .pem file"
    cp $installation_path/APPLICATIVE/SSL/*.pem $targetDir/nb-infra/ssl/
fi
keyFiles=$(ls $installation_path/APPLICATIVE/SSL/*.key 2> /dev/null | wc -l)
echo "keyFiles:"$keyFiles
if [[ $keyFiles -gt 0 ]]
then
    echo "Copying .key file"
    cp $installation_path/APPLICATIVE/SSL/*.key $targetDir/nb-infra/ssl/
fi