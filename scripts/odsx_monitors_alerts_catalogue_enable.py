    ...
    try:
        ...
        if len(alertDict)>0 :
            ...
            if choice=='99':
                quit()
            if choice=='1':
                alertNumber = str(input(Fore.YELLOW+"Enter alert id you want to enable : "+Fore.RESET))
                while(len(alertNumber)==0):
                    alertNumber = str(input(Fore.YELLOW+"Enter alert id you want to enable : "+Fore.RESET))
                alertName = alertDict.get(int(alertNumber))
                confirmInstall = str(input(Fore.YELLOW+"Are you sure want to enable alert ["+alertName+"] (y/n) [y]: "+Fore.RESET))
                if(len(str(confirmInstall))==0):
                    confirmInstall='y'
                if(confirmInstall=='y'):
                    enableKapacitorServiceByHost(alertName)
            if choice=="":
                confirmInstall = str(input(Fore.YELLOW+"Are you sure want to enable alert (y/n) [y]: "+Fore.RESET))
                if(len(str(confirmInstall))==0):
                    confirmInstall='y'
                if(confirmInstall=='y'):
