...
def enableKapacitorServiceByHost(alertName):
    ...
    with Spinner():
        ...
        if (output == 0):
            verboseHandle.printConsoleInfo("Alert "+str(alertName)+" enabled.")
        else:
            verboseHandle.printConsoleError("Alert "+str(alertName)+" unable to enabled.")
...
if __name__ == '__main__':
    verboseHandle.printConsoleWarning('Menu -> Monitors -> Alerts -> Services -> Kapacitor -> Enable')
    try:
        ...
        if len(alertDict)>0 :
            ...
            if choice=='99':
                quit()
            if choice=='1':
                alertNumber = str(userInputWrapper(Fore.YELLOW+"Enter alert id you want to enable : "+Fore.RESET))
                while(len(alertNumber)==0):
                    alertNumber = str(userInputWrapper(Fore.YELLOW+"Enter alert id you want to enable : "+Fore.RESET))
                alertName = alertDict.get(int(alertNumber))
                confirmInstall = str(userInputWrapper(Fore.YELLOW+"Are you sure want to enable alert ["+alertName+"] (y/n) [y]: "+Fore.RESET))
                if(len(str(confirmInstall))==0):
                    confirmInstall='y'
                if(confirmInstall=='y'):
                    enableKapacitorServiceByHost(alertName)
            if choice=="":
                confirmInstall = str(userInputWrapper(Fore.YELLOW+"Are you sure want to enable alert (y/n) [y]: "+Fore.RESET))
                if(len(str(confirmInstall))==0):
                    confirmInstall='y'
                if(confirmInstall=='y'):
