...
echo " kafkaBrokerHost1 "$2" kafkaBrokerHost2 "$3" kafkaBrokerHost3 "$4" counter ID "$5" installtelegrafFlag "$1" baseFolderLocation "$6
installtelegrafFlag=$1
kafkaBrokerHost1=$2
kafkaBrokerHost2=$3
kafkaBrokerHost3=$4
id=$5
baseFolderLocation=$6
dataFolderKafka=$7
dataFolderZK=$8
logsFolderKafka=${9}
logsFolderZK=${10}
...
echo " dataFolderKafka "$8" dataFolderZK "$9" logsFolderKafka "$logsFolderKafka" logsFolderZK "$logsFolderZK" installjavaFlag "$installjavaFlag
cd /dbagiga/
tar -xvf install.tar
home_dir=$(pwd)
...
mkdir -p $dataFolderKafka
  ...
  # adding properties
    ...
    echo "zookeeper.connect="$kafkaBrokerHost1":2181,"$kafkaBrokerHost3":2181,"$kafkaBrokerHost2":2181">>$KAFKAPATH/config/server.properties
    source setenv.sh
