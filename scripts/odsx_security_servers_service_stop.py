...
from utils.ods_cluster_config import config_get_service_list_with_status, config_get_service_hosts_list
...
def exitAndDisplay(isMenuDriven):
    if(isMenuDriven=='m'):
        os.system('python3 scripts/odsx_security_servers_service_stop.py'+' '+isMenuDriven)
    else:
        cliArgumentsStr=''
        for arg in cliArguments:
            cliArgumentsStr+=arg
            cliArgumentsStr+=' '
        os.system('python3 scripts/odsx_security_servers_service_stop.py'+' '+cliArgumentsStr)

def stopSecureServiceServer(argsString):
    os.system('python3 scripts/servers_manager_scriptbuilder.py '+argsString)

if __name__ == '__main__':
    logger.info("security - service - stop ")
    verboseHandle.printConsoleWarning('Menu -> Servers -> Service -> Stop')
    args = []
    ...
    try:
        ...
        if(serverStartType=='1'):
            ...
            if(optionMainMenu != 99):
                if len(streamDict) >= optionMainMenu:
                    serviceStart = streamDict.get(optionMainMenu)
                    ...
                    if(choice.casefold()=='no' or choice.casefold()=='n'):
                        ...
                        args.append('--host')
                        args.append(os.getenv(serviceStart.ip))
                        args.append('-u')
                        args.append(user)
                        args = str(args)
            ...
            if(confirm=='yes' or confirm=='y'):
                ...
                serviceLength = len(serviceHosts)+1
                with Spinner():
                    with ThreadPoolExecutor(serviceLength) as executor:
                        for host in serviceHosts:
                            ...
                            executor.submit(stopSecureServiceServer,argsString)
        ...
        logger.error("Invalid argument service stop :"+str(e))
