...
def loadConfigurations():
    fileName = "cdc_tables_online.json"
    logger.info("loadConfigurations()")
    logger.info("url : " + "http://" + managerHost + ":8090/v2/data-integration/di-consumer/load-configuration")
    json_object = json.dumps(getDataPURESTConsumerConfiguration(), indent=4)
    ...
    response = requests.put("http://" + managerHost + ":8090/v2/data-integration/di-consumer/load-configuration",
                            files=filesToUpload, headers=headers)
    ...
    if response.status_code == 201:
        ...
        verboseHandle.printConsoleError("Unable to load configuration : " + responseCode)
...
def getDataPURESTConsumerConfiguration():
    data = {
        "generic": {
            "diConsumerName": "cdc_tables"
        },
        "cdc": {
            "hostName": "" + cdchostname + "",
            "port": 2050,
            "type": "CR8"
        },
        "kafka": {
            "brokers": "" + kafkabrokers + "",
            "topicName": "cdc_tables_online",
            "topicPartitions": int(kafkatopicPartitions),
            "topicReplicationFactor": int(kafkatopicReplicationFactor),
            "kafkaMaxPollRecords": int(kafkaMaxPollRecords),
            "kafkaReceiveBufferConfig": int(kafkaReceiveBufferConfig)
        },
        "space": {
            "spaceName": "bllspace"
        }
...
def proceedToDeployPUInputParam(managerHost):
    ...
    global cdchostname
    global kafkabrokers
    global kafkatopicPartitions
    global kafkatopicReplicationFactor
    global kafkaMaxPollRecords
    global kafkaReceiveBufferConfig
    ...
    resourceName = str(
        userInputWrapper(Fore.YELLOW + "Enter name of PU to deploy [cdc_tables-dih-consumer-online.war] :" + Fore.RESET))
    ...
    processingUnitName = str(userInputWrapper(Fore.YELLOW + "Enter Resource Name [consumer-fullsync] : " + Fore.RESET))
    while (len(str(processingUnitName)) == 0):
        processingUnitName = 'consumer-fullsync'
...
if __name__ == '__main__':
    verboseHandle.printConsoleWarning('Menu -> Data Engine -> CR8 pipelines -> Consumer -> Online -> Consumer Deploy')
    try:
