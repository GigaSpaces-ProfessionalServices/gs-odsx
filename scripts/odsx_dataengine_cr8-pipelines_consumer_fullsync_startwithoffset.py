...
def proceedTostartConsumer(spaceNodes):
    ...
    offsetVal = str(
        userInputWrapper(Fore.YELLOW + "Enter offset [end] :" + Fore.RESET))
    ...
    parallelism = str(
        userInputWrapper(Fore.YELLOW + "parallelism [12] :" + Fore.RESET))
    if (len(str(parallelism)) == 0):
        parallelism = 12
    cleanStart = str(
        userInputWrapper(Fore.YELLOW + "cleanStart [false] :" + Fore.RESET))
    if (len(str(cleanStart)) == 0):
        cleanStart = "false"
    if cleanStart.lower() == "true":
        cleanStart = True
    else:
        cleanStart = False
    data = {
    ...
    with Spinner():
        response = requests.post(
            "http://" + managerHost + ":8090/v2/data-integration/di-consumer/" + consumerName + "/start",
...
if __name__ == '__main__':
    verboseHandle.printConsoleWarning('Menu -> Data Engine -> CR8 pipelines -> Consumer -> Fullsync -> Consumer start with offset')
    try:
