...
def exitAndDisplay(isMenuDriven):
    ...
    if(isMenuDriven=='m'):
        ...
        os.system('python3 scripts/odsx_security_servers_service_start.py'+' '+isMenuDriven)
    else:
        cliArgumentsStr=''
        for arg in cliArguments:
            cliArgumentsStr+=arg
            cliArgumentsStr+=' '
        os.system('python3 scripts/odsx_security_servers_service_start.py'+' '+cliArgumentsStr)

def startServiceServer(argsString):
    os.system('python3 scripts/servers_manager_scriptbuilder.py '+argsString)

if __name__ == '__main__':
    logger.info("security - service - start ")
    verboseHandle.printConsoleWarning('Menu -> Servers -> Service -> Start')
    args = []
    ...
    try:
                ...
                with Spinner():
                    with ThreadPoolExecutor(serviceLength) as executor:
                        for host in serviceHosts:
                            ...
                            executor.submit(startServiceServer,argsString)
        ...
        logger.error("Invalid argument service start :"+str(e))
