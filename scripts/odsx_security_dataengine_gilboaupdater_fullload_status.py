...
from utils.odsx_db2feeder_utilities import getMSSQLQueryStatusFromSqlLite, getUsernameByHost, getPasswordByHost
...
def inputParam():
    ...
    if(str(inputChoice)=='1'):
        inputNumberToStatus = str(userInputWrapper(Fore.YELLOW+"Enter serial number to status mssql-feeder : "+Fore.RESET))
        if(len(str(inputNumberToStatus))==0):
            inputNumberToStatus = str(userInputWrapper(Fore.YELLOW+"Enter serial number to get status mssql-feeder : "+Fore.RESET))
...
def proceedToGetStatusMSSQLFeeder(puName):
    ...
    queryStatus = str(getMSSQLQueryStatusFromSqlLite(puName)).replace('"', '')
    verboseHandle.printConsoleInfo(puName + " : " + queryStatus)

if __name__ == '__main__':
    logger.info("odsx_security_dataengine_mssql-feeder_status")
    verboseHandle.printConsoleWarning("Menu -> DataEngine  -> Gilboa -> Full load -> Status")
    username = ""
    password = ""
