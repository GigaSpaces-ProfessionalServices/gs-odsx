...
def createGSCInputParam(managerNodes,spaceNodes,managerHostConfig):
    ...
    try:
        confirmCreateGSC = str(userInputWrapper(Fore.YELLOW+"Do you want to create GSC ? (y/n) [y] :"+Fore.RESET))
        ...
        if(confirmCreateGSC=='y'):
            ...
            numberOfGSC = str(userInputWrapper(Fore.YELLOW+"Enter number of GSC per host [2] :"+Fore.RESET))
            if(len(str(numberOfGSC))==0):
                numberOfGSC=2
            logger.info("numberofGSC :"+str(numberOfGSC))

            memoryGSC = str(userInputWrapper(Fore.YELLOW+"Enter memory to create gsc [12g] :"+Fore.RESET))
            if(len(memoryGSC)==0):
                memoryGSC="12g"

            zoneGSC = str(userInputWrapper(Fore.YELLOW+"Enter zone [bll] :"+Fore.RESET))
...
def dataPuREST(resource,resourceName,zone,partition,maxInstancesPerMachine,backUpRequired):
    ...
    try:
        ...
        slaProperties = str(userInputWrapper(Fore.YELLOW+"Enter pu.autogenerated-instance-sla value [true] :"+Fore.RESET))
        ...
        tieredCriteriaConfigFilePathInput = str(userInputWrapper(Fore.YELLOW+"Enter tieredCriteriaConfig.filePath ["+str(tieredCriteriaConfigFilePath)+"]: "+Fore.RESET))
        ...
        global spaceNameCfg
        spaceNameCfg = str(userInputWrapper(Fore.YELLOW+"Enter space name to set space.name [bllspace] : "+Fore.RESET))
...
def copyFile(hostips, srcPath, destPath, dryrun=False):
    ...
    for hostip in hostips:
        if scp_upload(hostip, user, srcPath, destPath):
            ...
            logger.info(
                "Done copying, hostip=" + hostip + ", username=" + user + ", srcPath=" + srcPath + ", destPath=" + destPath)
        else:
            return False
    return True
...
def proceedForTieredStorageDeployment(managerHostConfig,confirmCreateGSC):
    ...
    try:
        ...
        global resourceName
        resourceName = str(userInputWrapper(Fore.YELLOW+"Enter name of PU to deploy [bllservice] :"+Fore.RESET))
        if(len(str(resourceName))==0):
          resourceName='bllservice'
        logger.info("nameOfPU :"+str(resourceName))

        global partition
        partition = str(userInputWrapper(Fore.YELLOW+"Enter partition required [50] :"+Fore.RESET))
        ...
        global zoneOfPU
        zoneOfPU = str(userInputWrapper(Fore.YELLOW+"Enter zone of processing unit to deploy [bll] :"+Fore.RESET))
        if(len(str(zoneOfPU))==0):
            zoneOfPU = 'bll'
        logger.info("Zone Of PU :"+str(zoneOfPU))

        global maxInstancesPerMachine
        maxInstancesPerMachine = str(userInputWrapper(Fore.YELLOW+"Enter maxInstancesPerMachine to deploy [1] :"+Fore.RESET))
        ...
        backUpRequired = str(userInputWrapper(Fore.YELLOW+"SLA [HA] ? (y/n) [y] :"+Fore.RESET))
...
if __name__ == '__main__':
    ...
    verboseHandle.printConsoleWarning("Menu -> Security -> Dev -> TieredStorage -> Deploy")
    username = ""
    password = ""
    try:
