...
def listDeployed(managerHost):
    ...
    try:
        ...
        verboseHandle.printConsoleInfo(db_file)
...
def inputParam():
    ...
    if(str(inputChoice)=='1'):
        inputNumberToStop = str(userInputWrapper(Fore.YELLOW+"Enter serial number to stop mysql-feeder : "+Fore.RESET))
        if(len(str(inputNumberToStop))==0):
            inputNumberToStop = str(userInputWrapper(Fore.YELLOW+"Enter serial number to stop mysql-feeder : "+Fore.RESET))
        proceedToStopMysqlFeeder(inputNumberToStop)
    if(len(str(inputChoice))==0):
        elements = len(fileNameDict)
        for i in range (1,elements+1):
            proceedToStopMysqlFeeder(str(i))
...
def proceedToStopMysqlFeeder(fileNumberToStop):
    logger.info("proceedToStopMysqlFeeder()")
    #shFileName = fileNameDict.get(str(fileNumberToStop))
    puName = gs_space_dictionary_obj.get(str(fileNumberToStop))
    proceedToStopMysqlFeederbyName(puName)

def proceedToStopMysqlFeederbyName(puName):
    logger.info("proceedToStopMysqlFeeder()")
    #shFileName = fileNameDict.get(str(fileNumberToStop))
...
if __name__ == '__main__':
    logger.info("odsx_dataengine_mysql-feeder_stop")
    verboseHandle.printConsoleWarning("Menu -> DataEngine -> Mysql-Feeder -> Stop")
    try:
        managerHost=''
        ...
        if(len(str(managerHost))>0):
            ...
            if(len(str(gs_space_dictionary_obj))>2):
                if len(sys.argv) > 1 and sys.argv[1] != "m":
                    proceedToStopMysqlFeederbyName(sys.argv[1])
                else:
                    inputParam()
            else:
        ...
        verboseHandle.printConsoleError("Eror in odsx_mysql-feeder_stop : "+str(e))
        handleException(e)