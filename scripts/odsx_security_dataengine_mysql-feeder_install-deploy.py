...
from utils.odsx_db2feeder_utilities import getPortNotExistInMysqlFeeder
...
def listDeployed(managerHost):
    ...
    try:
        ...
        for data in jsonArray:
            if(str(data["name"]).casefold().__contains__("mysqlfeeder")):
...
def createGSCInputParam():
    ...
    numberOfGSC = str(readValuefromAppConfig("app.dataengine.mysql-feeder.gscpercluster"))
    ...
    memoryGSC = str(readValuefromAppConfig("app.dataengine.mysql-feeder.gsc.memory"))
...
def updateAndCopyJarFileFromSourceToShFolder(puName):
    ...
    head , tail = os.path.split(sourceMysqlJarFilePath)
    fileName= str(tail).replace('.jar','')
    filePrefix = fileName
    targetJarFile=sourceMysqlFeederShFilePath+fileName+puName+'.jar'
    userCMD = os.getlogin()
    if userCMD == 'ec2-user':
        cmd = "sudo cp "+sourceMysqlJarFilePath+' '+sourceMysqlFeederShFilePath+fileName+puName+'.jar'
        #print(cmd)
    else:
        cmd = "cp "+sourceMysqlJarFilePath+' '+sourceMysqlFeederShFilePath+fileName+puName+'.jar'
...
def uploadFileRestAll(managerHostConfig,feederName):
    verboseHandle.printConsoleWarning("Proceeding for : "+sourceMysqlJarFilePath)
    logger.info("url : "+"curl -X PUT -F 'file=@"+str(sourceMysqlJarFilePath)+"' http://"+managerHostConfig+":8090/v2/pus/resources")
    status = os.system("curl -X PUT -F 'file=@"+str(sourceMysqlJarFilePath)+"' http://"+managerHostConfig+":8090/v2/pus/resources -u "+username+":"+password+"")
    print("\n")
...
def uploadFileRest(managerHostConfig,feederName):
    try:
        ...
        for file in glob.glob("load_*.sh"):
            os.chdir(directory)
            exitsFeeder = str(file).replace('load','mysqlfeeder').replace('.sh','').casefold()
...
def getDataPUREST(resource,resourceName,zoneOfPU,restPort,managerHost):
    backUpRequired=0
    mysqlHost = str(readValueByConfigObj("app.dataengine.mysql-feeder.mysql.server")).replace('"','').replace(' ','')
    mysqlUser = str(readValueByConfigObj("app.dataengine.mysql-feeder.mysql.username")).replace('"','').replace(' ','')
    mysqlPassword = str(readValueByConfigObj("app.dataengine.mysql-feeder.mysql.password")).replace('"','').replace(' ','')
    mysqlDatabase = str(readValueByConfigObj("app.dataengine.mysql-feeder.mysql.databasename")).replace('"','').replace(' ','')
    data={
        "resource": ""+resource+"",
        "topology": {
            "instances": 1,
...
def sqlLiteCreateTableDB():
    ...
    try:
        db_file = str(readValueByConfigObj("app.dataengine.mysql-feeder.sqlite.dbfile")).replace('"','').replace(' ','')
        ...
        cnx.execute("CREATE TABLE IF NOT EXISTS mysql_host_port (file VARCHAR(50), feeder_name VARCHAR(50), host VARCHAR(50), port varchar(10))")
        cnx.commit()
        cnx.close()
    except Exception as e:
        handleException(e)

def createMysqlEntryInSqlLite(puName, file, restPort):
    logger.info("createMysqlEntryInSqlLite()")
    try:
        ...
        db_file = str(readValueByConfigObj("app.dataengine.mysql-feeder.sqlite.dbfile")).replace('"','').replace(' ','')
        cnx = sqlite3.connect(db_file)
        cnx.execute("INSERT INTO mysql_host_port (file, feeder_name, host, port) VALUES ('"+str(file)+"', '"+str(puName)+"','"+str(hostId)+"','"+str(restPort)+"')")
        cnx.commit()
        cnx.close()
    except Exception as e:
        handleException(e)

def newInstallMysqlFeeder():
    global newInstall
    newInstall=[]
    directory = os.getcwd()
    os.chdir(sourceMysqlFeederShFilePath)
...
def getHardLimitMemoryInBytes():
    hardLimit = str(readValueByConfigObj("app.dataengine.mysql-feeder.hard.limit"))
    if hardLimit[-1] == "m":
        ...
        logger.info("mysql-feeder Hard Limit value is "+ hardLimit + " Enter Hard Limit value like 100m or 1g")
        verboseHandle.printConsoleInfo("mysql-feeder Hard Limit value is "+ hardLimit + " Enter Hard Limit value like 100m or 1g")
        exit(0)
...
def proceedToDeployPU(feederName):
    ...
    try:
        ...
        os.chdir(sourceMysqlFeederShFilePath)
        #os.system("pwd")
        restPort = str(readValueByConfigObj("app.dataengine.mysql-feeder.rest.port")).replace('"','').replace(' ','')
            ...
            if exitsFeeder not in activefeeder:
              ...
              #  logger.info("filePrefix : "+filePrefix)
               # resource = filePrefix+'_'+puName+'.jar'
                resource = str(getYamlFilePathInsideFolder(".mysql.jars.mysqlJarFile"))
                puName = 'mysqlfeeder_'+puName
                port = getPortNotExistInMysqlFeeder(restPort)
                    ...
                    with Spinner():
                        while(retryCount>0 or (not str(status).casefold().__contains__('successful')) or (not str(status).casefold().__contains__('failed'))):
                            ...
                            if(str(status).casefold().__contains__('successful')):
                                time.sleep(2)
                                createMysqlEntryInSqlLite(puName,file,restPort)
                                verboseHandle.printConsoleInfo("Entry : "+str(file)+" puName :"+str(puName))
                                userCMD = os.getlogin()
                                if userCMD == 'ec2-user':
                                    cmd = "sudo rm -f "+sourceMysqlFeederShFilePath+resource
                                else:
                                    cmd = "rm -f "+sourceMysqlFeederShFilePath+resource
                                logger.info("cmd : "+str(cmd))
                                #home = executeLocalCommandAndGetOutput(cmd)
                                break
...
def displaySummaryOfInputParam():
    ...
    verboseHandle.printConsoleInfo("Enter mysql.server : "+str(mysqlServer))
    verboseHandle.printConsoleInfo("Enter feeder.writeBatchSize : "+str(feederWriteBatchSize))
    verboseHandle.printConsoleInfo("Enter mysql-feeder.hard.limit: " + str(readValueByConfigObj("app.dataengine.mysql-feeder.hard.limit")))
    ...
    verboseHandle.printConsoleInfo("Enter source file path of mysql-feeder .jar file including file name : "+str(sourceMysqlJarFilePath))
    verboseHandle.printConsoleInfo("Enter source file path of mysql-feeder *.sh file : "+str(sourceMysqlFeederShFilePath))
    #verboseHandle.printConsoleInfo("Enter source file of mysql-feeder *.sh file : "+str(newInstall))

def proceedToDeployPUInputParam(managerHost):
    logger.info("proceedToDeployPUInputParam()")

    global sourceMysqlJarFilePath
    sourceMysqlJarFileConfig = str(getYamlFilePathInsideFolder(".mysql.jars.mysqlJarFile"))
    #print(Fore.YELLOW+"Enter source file path of mysql-feeder .jar file including file name ["+sourceMysqlJarFileConfig+"] : "+Fore.RESET)
    #if(len(str(sourceMysqlJarFilePath))==0):
    sourceMysqlJarFilePath = sourceMysqlJarFileConfig
    #set_value_in_property_file("app.dataengine.mysql-feeder.jar",sourceMysqlJarFilePath)

    global sourceMysqlFeederShFilePath
    sourceInstallerDirectory = str(os.getenv("ODSXARTIFACTS"))
    logger.info("sourceInstallerDirectory:"+sourceInstallerDirectory)
    sourceMysqlFeederShFilePathConfig = str(sourceInstallerDirectory+".mysql.scripts.").replace('.','/')

    #print(Fore.YELLOW+"Enter source file path (directory) of *.sh file ["+sourceMysqlFeederShFilePathConfig+"] : "+Fore.RESET)
    #if(len(str(sourceMysqlFeederShFilePath))==0):
    sourceMysqlFeederShFilePath = sourceMysqlFeederShFilePathConfig
    lastChar = str(sourceMysqlFeederShFilePath[-1])
    logger.info("last char:"+str(lastChar))
    if(lastChar!='/'):
        sourceMysqlFeederShFilePath = sourceMysqlFeederShFilePath+'/'
    #set_value_in_property_file("app.dataengine.mysql-feeder.filePath.shFile",sourceMysqlFeederShFilePath)

    #uploadFileRest(managerHost)
    newInstallMysqlFeeder()
    global partition
    partition='1'
    ...
    global spaceName
    spaceName = str(readValueByConfigObj("app.dataengine.mysql-feeder.space.name"))
    #print(Fore.YELLOW+"Enter space.name ["+spaceName+"] : "+Fore.RESET)
    #if(len(str(spaceName))==0):
    #    spaceName='bllspace'

    global mysqlServer
    mysqlServerConfig = str(readValueByConfigObj("app.dataengine.mysql-feeder.mysql.server"))
    #print(Fore.YELLOW+"Enter mysql.server ["+mysqlServerConfig+"]: "+Fore.RESET)
    #if(len(str(mysqlServer))==0):
    mysqlServer = mysqlServerConfig
    # set_value_in_property_file("app.dataengine.mysql-feeder.mysql.server",mysqlServer)

    global feederWriteBatchSize
    feederWriteBatchSize = str(readValueByConfigObj("app.dataengine.mysql-feeder.writeBatchSize"))
    ...
    feederSleepAfterWrite = str(readValueByConfigObj("app.dataengine.mysql-feeder.sleepAfterWriteInMillis"))
    ...
    if(feederStartType=='1'):
        ...
        if len(feeder_dict_obj) >= optionMainMenu:
            ...
            if(len(str(finalConfirm))==0):
                finalConfirm='y'
            if(finalConfirm=='y'):
                logger.info("mysql feeder confirmCreateGSC "+confirmCreateGSC)
...
def displayAvailableFeederList():
    ...
    for file in glob.glob("load_*.sh"):
        ...
        puName = str(file).replace('load_', '').replace('.sh', '').casefold()
        dataArray = [Fore.GREEN + str(counter + 1) + Fore.RESET,
                     Fore.GREEN + 'mysqlfeeder_'+ puName + Fore.RESET
                     ]
...
if __name__ == '__main__':
    logger.info("odsx_dataengine_mysql-feeder_install")
    verboseHandle.printConsoleWarning('Menu -> DataEngine -> Mysql-Feeder -> Install-Deploy')
    username = ""
    password = ""
    appId=""
    safeId=""
            ...
            if(len(str(managerNodes))>0):
                ...
                if(len(str(managerHost))>0):
                    ...
                    if(len(space_dict_obj)>0):
                        confirmCreateGSC = str(readValuefromAppConfig("app.dataengine.mysql-feeder.gsc.create"))
