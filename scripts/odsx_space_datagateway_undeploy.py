...
def getUserInput(managerHost):
    ...
    try:
        from utils.odsx_keypress import userInputWrapper
        typeOfRemove = str(userInputWrapper(Fore.YELLOW+"[1] For individual undeployed PU\n[Enter] For all above undeployed PUs \n[99] For exist. :"+Fore.RESET))
        print(typeOfRemove)
...
def listDeployed(managerHost):
    global gs_dg_dictionary_obj
    try:
        logger.info("managerHost :"+str(managerHost))
        response = requests.get("http://"+str(managerHost)+":8090/v2/pus/")
        logger.info("response status of host :"+str(managerHost)+" status :"+str(response.status_code)+" Content: "+str(response.content))
        jsonArray = json.loads(response.text)
        verboseHandle.printConsoleWarning("Data gateways on cluster:")
        headers = [Fore.YELLOW+"Sr No."+Fore.RESET,
                   Fore.YELLOW+"Name"+Fore.RESET,
                   Fore.YELLOW+"Instances"+Fore.RESET
                   ]
        gs_dg_dictionary_obj = host_dictionary_obj()
        counter=0
        dataTable=[]
        for data in jsonArray:
            ...
            gs_dg_dictionary_obj.add(str(counter+1),str(data["name"]))
            counter=counter+1
            dataTable.append(dataArray)
        printTabular(None,headers,dataTable)
        return gs_dg_dictionary_obj
    except Exception as e:
        handleException(e)
...
def proceedForAllUndeploy(managerHost):
    ...
    try:
        for key,value in gs_dg_dictionary_obj.items():
            logger.info(str(key)+" : "+str(value))
            spaceTobeUndeploy = gs_dg_dictionary_obj.get(str(key))
            print(spaceTobeUndeploy)
...
def proceedToUndeployPU(managerHost):
    ...
    try:
        from utils.odsx_keypress import userInputWrapper
        typeOfRemove = str(userInputWrapper(Fore.YELLOW+"[1] For individual undeploy data gateway\n[Enter] For all above data gateways \n[99] For exist. :"+Fore.RESET))
        logger.info("typeOfRemove : "+str(typeOfRemove))

        if(typeOfRemove=='1'):
            from utils.odsx_keypress import userInputWrapper
            spaceNumberTobeRemove = str(userInputWrapper(Fore.YELLOW+"Enter data gateway number to be remove : "+Fore.RESET))
            ...
            spaceTobeUndeploy = gs_dg_dictionary_obj.get(spaceNumberTobeRemove)
...
if __name__ == '__main__':
    verboseHandle.printConsoleWarning("Menu -> Space -> Data Gateway -> Undeploy")
    try:
        ...
        if(len(str(managerNodes))>0):
            ...
            if(len(str(managerHost))>0):
                ...
                gs_dg_dictionary_obj = listDeployed(managerHost)
                if(len(gs_dg_dictionary_obj)>0):
                    proceedToUndeployPU(managerHost)
                else:
                    logger.info("No data gateway pu found.")
                    verboseHandle.printConsoleInfo("No data gateway pu found.")
