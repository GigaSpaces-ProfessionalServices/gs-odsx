...
def createGSC(host):
    try:
        ...
        cmd = "cd; home_dir=$(pwd); source $home_dir/setenv.sh;$GS_HOME/bin/gs.sh container create --zone " + str(
...
def proceedForNewBackUpRollingUpdate(gscNumberToBeRollingUpdate):
    ...
    try:
        ...
        loggerTiered.info("Space ID to be restarted : " + str(spaceIdToBeRestarted) + "  ContainerId : " + str(
            containerIdToBeRestarted))
        response = requests.delete("http://"+managerHost+":8090/v2/containers/"+str(containerIdToBeRestarted),headers=requestHeader)
        ...
        response = requests.get("http://"+managerHost+":8090/v2/requests")
        jsonArray = json.loads(response.text)
        backUPResponseCode=""
        if(len(jsonArray)>1):
            jsonArray = json.loads(response.text)
...
def proceedForBackupRollingUpdate(gscNumberToBeRollingUpdate):
    ...
    try:
        ...
        response = requests.delete("http://"+managerHost+":8090/v2/containers/"+str(backupContainerId),headers=requestHeader)
