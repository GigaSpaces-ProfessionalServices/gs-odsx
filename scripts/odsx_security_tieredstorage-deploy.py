        ...
        if(individualHostConfirm=='n'):
            with Spinner():
                ...
                for i in range(1,len(space_dict_obj)+1):
                    ...
                    cmd = "cd; home_dir=$(pwd); source $home_dir/setenv.sh;$GS_HOME/bin/gs.sh --username="+usernameSpaceHost+" --password="+passwordSpaceHost+" container create --zone "+str(zoneGSC)+" --count "+str(numberOfGSC)+" --memory "+str(memoryGSC)+" "+str(host)+""
