...
def sqlLiteDeleteTableMsSqlFeeder():
    ...
    try:
        ...
        cnx.execute("DELETE FROM gilboa_host_port")
        verboseHandle.printConsoleInfo("Table : gilboa_host_port reseted.")
        cnx.commit()
        cnx.close()
...
if __name__ == '__main__':
    logger.info("odsx_dataengine_Gilboa-feeder_reset")
    verboseHandle.printConsoleWarning("Menu -> DataEngine  -> Gilboa -> Update -> Reset")
    try:
        sqlLiteDeleteTableMsSqlFeeder()
    except Exception as e:
        verboseHandle.printConsoleError("Eror in odsx_gilboa-feeder_reset : "+str(e))
        handleException(e)
