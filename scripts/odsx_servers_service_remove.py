...
def execute_scriptBuilder(host):
    ...
    commandToExecute="scripts/servers_service_remove.sh"
    additionalParam = removeJava+' '+removeUnzip
    logger.info("additionalParam : "+str(additionalParam))
    outputShFile = connectExecuteSSH(host, user,commandToExecute,additionalParam)
    print(outputShFile)
    logger.info("Output : scripts/servers_service_remove.sh :"+str(outputShFile))
...
def exitAndDisplay(isMenuDriven):
    ...
    if(isMenuDriven=='m'):
        ...
        os.system('python3 scripts/odsx_servers_service_remove.py'+' '+isMenuDriven)
    else:
        cliArgumentsStr=''
        for arg in cliArguments:
            cliArgumentsStr+=arg
            cliArgumentsStr+=' '
        os.system('python3 scripts/odsx_servers_service_remove.py'+' '+cliArgumentsStr)

def removeServiceServer(host,args,menuDrivenFlag,user):
    logger.info("BEFORE")
...
if __name__ == '__main__':
    logger.info("odsx_servers_service_remove")
                ...
                with Spinner():
                    with ThreadPoolExecutor(serviceHostsLength) as executor:
                        for host in serviceHosts:
                            executor.submit(removeServiceServer,host,args,menuDrivenFlag,user)
