...
def exitAndDisplay(isMenuDriven):
    if(isMenuDriven=='m'):
        os.system('python3 scripts/odsx_servers_service_stop.py'+' '+isMenuDriven)
    else:
        cliArgumentsStr=''
        for arg in cliArguments:
            cliArgumentsStr+=arg
            cliArgumentsStr+=' '
        os.system('python3 scripts/odsx_servers_service_stop.py'+' '+cliArgumentsStr)

def stopServiceServer(argsString):
    os.system('python3 scripts/servers_manager_scriptbuilder.py '+argsString)

if __name__ == '__main__':
    logger.info("servers - service - stop ")
                ...
                with Spinner():
                    with ThreadPoolExecutor(serviceHostsLength) as executor:
                        for host in serviceHosts:
                            ...
                            executor.submit(stopServiceServer,argsString)
