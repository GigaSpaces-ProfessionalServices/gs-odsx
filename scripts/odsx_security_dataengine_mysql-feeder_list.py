...
from utils.odsx_db2feeder_utilities import getMySqlQueryStatusFromSqlLite, getPasswordByHost, getUsernameByHost
        ...
        if (len(jsonArray) > 0):
            ...
            for data in jsonArray:
                ...
                if (str(data["name"]).__contains__("mysqlfeeder")):
                    ...
                    logger.info("UPDATE mysql_host_port SET host='" + str(hostId) + "' where feeder_name like '%" + str(
                        data["name"]) + "%' ")
                    mycursor = cnx.execute(
                        "UPDATE mysql_host_port SET host='" + str(hostId) + "' where feeder_name like '%" + str(
                            data["name"]) + "%' ")
...
if __name__ == '__main__':
    logger.info("odsx_dataengine_mysql-feeder_list")
    verboseHandle.printConsoleWarning("Menu -> DataEngine -> Mysql-Feeder -> List")
    username = ""
    password = ""
