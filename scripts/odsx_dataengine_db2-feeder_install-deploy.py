...
def listDeployed(managerHost):
    ...
    try:
        ...
        for data in jsonArray:
            if(str(data["name"]).casefold().__contains__("db2feeder")):
...
def proceedToCreateGSC(zoneGSC):
    ...
    for host in spaceNodes:
        commandToExecute = "cd; home_dir=$(pwd); source $home_dir/setenv.sh;$GS_HOME/bin/gs.sh container create --count="+str(numberOfGSC)+" --zone="+str(zoneGSC)+" --memory="+str(memoryGSC)+" --vm-option -Ddb2.jcc.charsetDecoderEncoder=3  "+str(os.getenv(host.ip))+" | grep -v JAVA_HOME"
...
def proceedToDeployPUInputParam(managerHost):
    ...
    #str(userInputWrapper(Fore.YELLOW+"Enter db2Password : "+Fore.RESET))
