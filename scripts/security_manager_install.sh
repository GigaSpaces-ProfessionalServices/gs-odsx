...
function installAirGapGS {
   ...
   #installation_file=$(find $gigasharepath/current/gs/*.zip -maxdepth 1)
...
#    echo $limitContentSoft>>/etc/security/limits.conf
   ...
   cp gigaspaces-smart-ods/config/security/security.properties $gigapath"/gs_config/"
   ...
   if [ "$gs_version_17" == "true" ]; then
      ...
      # -o: see the note on the airgap unzip above - stdin is the script body.
      unzip -qq -o $installation_file_16_4 -d  $targetDir
...
function gsCreateGSServeice {
  ...
  chmod 755 -R $GS_HOME/deploy/*
  if [ "$gs_version_17" == "true" ]; then
      chmod 755 -R $GS_HOME_16_4/tools/gs-webui/*
  else
